...
public class ImageController extends Controller {
    ...
    public static void showImage(int firstDir, int secondDir, int thirdDir, String imageName) {
        ...
        String originImagePath = IMAGE_ROOT + File.separator + "o" + File.separator + firstDir + File.separator + secondDir + File.separator + thirdDir + File.separator + originImageName + imageName.substring(imageName.indexOf("."));


        File toFile = new File(toImagePath);
        if (!toFile.exists()) {
            //创建缩略图
            play.libs.Images.resize(new File(originImagePath), toFile, width, height);
        }
        renderBinary(toFile);
    }
