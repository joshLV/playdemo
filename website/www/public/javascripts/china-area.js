...
(function ($) {
    $.fn.jChinaArea = function (o) {
        ...
        loc.fillOption(sCity, '0,' + $(":selected", sProvince).attr('loc'), o.s2);
        loc.fillOption(sCounty, '0,' + $(":selected", sProvince).attr('loc') + ',' + $(":selected", sCity).attr('loc'), o.s3);
        ...
        sProvince.change(function () {
            sCity.empty();
            loc.fillOption(sCity, '0,' + $(":selected", sProvince).attr('loc'));
            sCounty.empty();
            loc.fillOption(sCounty, '0,' + $(":selected", sProvince).attr('loc') + ',' + $(":selected", sCity).attr('loc'));
            if (o.aspnet) {
                writeInput();
            }
        })
