 ...
 function setUserInfo(){
  ...
  $("#userInfo.intrest[]").val(hobby.join(","));
...
//邮箱验证
 function rEmailC(){
	...
	if(user_email==''|| checkEmail(user_email)){
		$("#user_email").parent().parent().addClass('hover');
		return false;
	}else{
		$("#user_email").parent().parent().removeClass('hover');
	}
	$.post(
	      "user.php?act=isemail",
		  {user_email:user_email},
		  function(data){
			if(data.errno==1){
				
			}else{
				$("#showemailcheck").html('<p class="attention" style="margin-top:5px;">该邮箱地址已经存在！</p>');
				$("#user_email").parent().parent().addClass('hover');
			}
		  },
		  "json"
 ...
 function changeTel(telidentify){
   if(telidentify==1){
	 ...
	 if(oldtelnum&&checkTel(oldtelnum)){
	    $("#showoldtelcheck").html("<div class='chphone_msg'><div class='attention'><img  src='/public/images/error.gif' style='vertical-align:middle'/>&nbsp; * 您的手机号码有误 </div></div>");
	    return false;
	 }
	 if(newtelnum==''||checkTel(newtelnum)){
	   $("#shownewtelcheck").html("<div class='chphone_msg'><div class='attention'><img  src='/public/images/error.gif' style='vertical-align:middle'/>&nbsp; * 请输入您的11位新手机号码 </div></div>");
	   return false;
	 }
	 if(telcheckcode==''){
	   $("#showchangecode").html("<div class='chphone_msg'><div class='attention'><img  src='/public/images/error.gif' style='vertical-align:middle'/>&nbsp; * 您必须输入验证码 </div></div>");
	   return false;
	 }
	  $("#newtelnum").attr("disabled","");
	  $.post(
		      "user.php?act=checkchangetel",
			  {oldtelnum:oldtelnum,newtelnum:newtelnum,telcheckcode:telcheckcode,telidentify:1},
              function(data){
					if(data.errno==1){
					 $("#layout").hide();
					 $("#changeseccess").show();
					  //修改成功跳转
					  setTimeout(function(){ location.href = "user.php?act=setuserinfo"},10);
	   ...
	   if(bindtelnum =='' || checkTel(bindtelnum)){
	      $("#showbindtelcheck").val('手机格式错误');
		  return false;
	   }
	   if(bindcheckcode==''){
	       alert('验证码不能为空');
	   ...
	   $.post("/user/send",{bindtelnum:bindtelnum,telcheckcode:bindcheckcode,telidentify:2},function(data){
	            if(data.errno==0){
				 $("#layout").hide();
				 $("#changeseccess").show();
				  //修改成功跳转
...
function getBindCode(codeidy){
 if(codeidy==1){
  ...
  $("#getBindCode").attr("disabled","disabled");
   $.post(
		  ...
		  function(data){
		    if(data==1){
			   ...
			   $("#bindtelnum").attr("disabled","disabled");
			}else if(data.errno==-1){
			   $("#showbindtelcheck").html("<span class='yellowInfo allstep_info'><b></b><em>网络错误，请重发</em></span>");
			   $("#getBindCode").attr("disabled","");
			}else if(data.errno==-2){
				alert("您的手机已经绑定过优卡，不能再次绑定。");
			   $("#getBindCode").attr("disabled","");
			}
		  },
		  "json"
     ...
     var bindtelnum = $.trim($("#newtelnum").val());
    if(bindtelnum==''|| checkTel(bindtelnum)){
    alert("手机号码有误");
    return false;
    }
   ...
   $.post(
          "user.php?act=sendchangetelcode",
		  {bindtelnum:bindtelnum},
		  function(data){
		    if(data.errno==1){
			   $("#shownewtelcheck").html("<span class='yellowInfo allstep_info'><b></b><em>验证码已成功发送，请查收！</em></span>"); 
			   $("#getchangeCode").attr("disabled","");
			   $("#newtelnum").attr("disabled","disabled");
			}else if(data.errno==-1){
			   $("#shownewtelcheck").html("<span class='yellowInfo allstep_info'><b></b><em>网络错误，请重发</em></span>");
			   $("#getchangeCode").attr("disabled","");
			}else if(data.errno==-2){
				alert("您的手机已经绑定过优卡，不能再次绑定。");
			   //$("#shownewtelcheck").html("<span class='yellowInfo allstep_info'><b></b><em>网络错误，请重发</em></span>");
...
function activationCard(){
   ...
   var cardreg  = /^\d{16}$/;
   if(cardnum==''||cardnum.match(cardreg) == null){
      alert("请输入16位的卡号");
	  return false;
   }
   $.post("user.php?act=checkactivationcard",{cardnum:cardnum},function(data){
      if(data.errno==1){
	     alert('恭喜你！优卡激活成功');
		 setTimeout(function(){ location.href = "user.php?act=cardmanage";},10);
	  }else if(data.errno==-1){
	     alert("激活失败");
	  }else if(data.errno==-2){
	     alert("您已经绑定过优卡或者输入的优卡无效");
	  }else if(data.errno==-3){
	     alert("请输入16位的卡号");
	  }
   },"json")
