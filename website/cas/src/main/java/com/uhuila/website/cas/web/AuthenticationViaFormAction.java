package com.uhuila.website.cas.web;
...
public class AuthenticationViaFormAction {
    ...
    public final void doEntry(final RequestContext context, Credentials credentials) throws Exception {
        ...
        if (org.apache.commons.lang.StringUtils.isEmpty(usernamePasswordCredentials.getUsername())) {
            usernamePasswordCredentials.setUsername(lastUsername);
        }
    ...
    public final String submit(final RequestContext context, final Credentials credentials, final MessageContext messageContext) throws Exception {
        ...
        // 记住用户名
        CookieGenerator remberCookieGenerator = new CookieGenerator();
        remberCookieGenerator.setCookieName("last_username");
        
        CookieGenerator uncheckRemberCookieGenerator = new CookieGenerator();
        uncheckRemberCookieGenerator.setCookieName("unremember_user");
        Boolean isRememberUsername = context.getRequestParameters().getBoolean("rememberUsername", false);
        System.err.println("isRememberUsername=" + isRememberUsername);
        if (isRememberUsername.booleanValue()) {
            UsernamePasswordCredentials usernamePasswordCredentials = (UsernamePasswordCredentials)credentials;
            String oldName = usernamePasswordCredentials.getUsername();
            System.err.println("username=" + oldName);
            
            final HttpServletResponse response = WebUtils.getHttpServletResponse(context);
            remberCookieGenerator.removeCookie(response);
            remberCookieGenerator.addCookie(response, oldName);
            uncheckRemberCookieGenerator.removeCookie(response);
        } else {
            final HttpServletResponse response = WebUtils.getHttpServletResponse(context);
            remberCookieGenerator.removeCookie(response);
            uncheckRemberCookieGenerator.removeCookie(response);
            uncheckRemberCookieGenerator.addCookie(response, "true");
        }
    ...
    private void addCookieValue(HttpServletRequest request, HttpServletResponse response, String key, String value) {
        Cookie[] cookies = request.getCookies();
        ...
        if (theCookie == null) {
            theCookie = new Cookie(key, value);
        }
        theCookie.setValue(value);
