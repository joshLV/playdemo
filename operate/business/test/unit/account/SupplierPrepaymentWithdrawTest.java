...
public class SupplierPrepaymentWithdrawTest extends UnitTest {
    ...
    public void 预付款足额消费状态测试() {
        ...
        assertBigDecimalEquals(BigDecimal.ZERO, 可提现金额(supplier));
        assertBigDecimalEquals(BigDecimal.ZERO, 预付款余额(supplier));
        
        创建预付款(beforeDays(20), afterDays(1), new BigDecimal(100));
        assertBigDecimalEquals(BigDecimal.ZERO, 账户余额(supplier));
        assertBigDecimalEquals(BigDecimal.ZERO, 可提现金额(supplier));
        assertBigDecimalEquals(new BigDecimal(100), 预付款余额(supplier));
        ...
        assertBigDecimalEquals(new BigDecimal(20), 可提现金额(supplier));
        assertBigDecimalEquals(new BigDecimal(100), 预付款余额(supplier));
    ...
    public void 预付款未完成消费状态测试() {
        ...
        assertBigDecimalEquals(new BigDecimal(100), 账户余额(supplier));
        assertBigDecimalEquals(new BigDecimal(40), 可提现金额(supplier));
        assertBigDecimalEquals(new BigDecimal(100), 预付款余额(supplier));
    }
    ...
    private BigDecimal 可提现金额(Supplier supplier) {
        return null;
    }

    private BigDecimal 预付款余额(Supplier supplier) {
        return null;
    }
    ...
    private void 创建预付款(final Date effactiveAt, final Date expireAt, final BigDecimal prepaymentAmount) {
        FactoryBoy.create(Prepayment.class, new BuildCallback<Prepayment>() {
            @Override
            public void build(Prepayment p) {
                p.effectiveAt  = effactiveAt;
                p.expireAt = expireAt;
                p.createdAt = effactiveAt;
