...
public class SecKillGoodsItemUnitTest extends UnitTest {
    ...
    public void 取得所有的秒杀活动信息() {
        ...
        FactoryBoy.batchCreate(5, SecKillGoodsItem.class, new SequenceCallBack<SecKillGoodsItem>() {
            @Override
            public void sequence(SecKillGoodsItem target, int seq) {
                target.goodsTitle = "第" + seq + "波" + seq;
                target.salePrice = new BigDecimal(seq);
                target.secKillBeginAt = new Date();
                target.secKillEndAt = DateHelper.afterMinuts(new Date(), seq);
        ...
        JPAExtPaginator<SecKillGoodsItem> secKillGoodsList = SecKillGoodsItem.findByCondition(condition, secKillGoods.id, 0, 10);
        assertEquals(5, secKillGoodsList.size());

        condition.goodsTitle = "第";
        condition.status = SecKillGoodsStatus.ONSALE;
        secKillGoodsList = SecKillGoodsItem.findByCondition(condition, secKillGoods.id, 0, 10);
        assertEquals(5, secKillGoodsList.size());
    ...
    public void testUpdateSecKillGoods() {
        ...
        SecKillGoodsItem secKillGoodsItem1 = SecKillGoodsItem.findById(secKillGoodsItem.id);
        assertEquals("秒杀活动一", secKillGoodsItem1.goodsTitle);
        assertEquals(SecKillGoodsStatus.OFFSALE, secKillGoodsItem1.status);

        SecKillGoodsItem.updateStatus(SecKillGoodsStatus.ONSALE, secKillGoodsItem.id);
        assertEquals(SecKillGoodsStatus.ONSALE, secKillGoodsItem1.status);
    }

    @Test
    public void testIsExpired() {
        SecKillGoodsItem item = FactoryBoy.create(SecKillGoodsItem.class, "expired", new BuildCallBack<SecKillGoodsItem>() {
            @Override
