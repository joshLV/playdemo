...
public class RbacMenuTest extends UnitTest {
    ...
    public void theNoDefinedNavigationWillBeDeleted() {
        OperateNavigation mainNav = OperateNavigation.find("byApplicationNameAndName", "traders-admin", "main").first();
        assertNotNull(mainNav);
        assertNotNull(mainNav.permissions);
        assertEquals(0, mainNav.permissions.size());
                
        OperateNavigation userNav = OperateNavigation.find("byApplicationNameAndName", "traders-admin", "user_add").first();
        assertNotNull(userNav);
        assertNotNull(userNav.permissions);
        assertEquals(2, userNav.permissions.size());
        
        // 加载后，数据库中没有在yml定义的导航记录必须被删除
        OperateNavigation toDeleteNav = OperateNavigation.find("byApplicationNameAndName", "traders-admin", "to_delete").first();
    ...
    public void canLoadNavigationYamlFile() {
        ...
        // 加载后，数据库中必须有相关的记录
        List<OperateNavigation> menus = OperateNavigation.find("byApplicationName", "traders-admin").fetch();
        assertTrue(menus.size() > 0);

        OperateNavigation mainNav = OperateNavigation.find("byApplicationNameAndName", "traders-admin", "main")
                .first();
        assertNotNull(mainNav);
        OperateNavigation homeNav = OperateNavigation.find("byApplicationNameAndName", "traders-admin", "home")
                .first();
