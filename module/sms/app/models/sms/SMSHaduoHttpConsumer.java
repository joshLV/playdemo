...
public class SMSHaduoHttpConsumer extends RabbitMQConsumer<SMSMessage> {
    ...
    private void saveJournal(SMSMessage message, String status, String serial) {
        for (String phone : message.getPhoneNumbers()) {
            new SMSJournal(message.getContent(), phone, status, null).save();
        }
    }
    ...
    protected void consume(SMSMessage message) {
        if (SEND_URL == null) {
            saveJournal(message, "0", null);
            Logger.debug("SMSHaduoHttpConsumer: can not get the SEND_URL in application.conf");
            return;
        }
        //准备url
